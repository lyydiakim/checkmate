...
const SharePage: React.FC = () => {
  ...
  const formatNameListtoString = (name: string) => {
    ...
    const formattedItems = Object.entries(items).map(([item, cost]) => `${item}: ${Math.ceil((cost / itemPrice[item].splitBetween)*100)/100}`);
      ...
      { nameList &&
        Object.keys(nameList).map((name, index) => (
          <div key={index} className="text-gray-200 mt-4 flex flex-col items-center">
            ...
            <ul>
              { nameList[name] &&
                Object.entries(nameList[name]).map(([item,cost], lineIndex) => (
                  <li key={lineIndex}>
            ...
            <CopyToClipboard text={formatNameListtoString(name)} onCopy={() => handleCopyList(name, index)}>
              <div
